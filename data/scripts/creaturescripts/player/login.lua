local function sendBoostMessage(player, category, isIncreased)
    return player:sendTextMessage(MESSAGE_BOOSTED_CREATURE, string.format("Event! %s is %screased. Happy Hunting!",
        category, isIncreased and "in" or "de"))
end
...
function playerLoginGlobal.onLogin(player)
    ...
    if player:getLastLoginSaved() == 0 then
        ...
        loginStr = string.format("Your last visit in %s: %s.", SERVER_NAME,
    ...
    -- Boosted
    player:sendTextMessage(MESSAGE_BOOSTED_CREATURE, string.format(
    ...
    if rewards > 0 then
        player:sendTextMessage(MESSAGE_LOGIN, string.format("You have %d reward%s in your reward chest.", rewards,
            rewards > 1 and "s" or ""))
    end
    ...
    local resultId = db.storeQuery("SELECT `recruiter` FROM `accounts` WHERE `id`= " ..
                                       getAccountNumberByPlayerName(getPlayerName(player)))
    if resultId then
    ...
    if configManager.getBoolean(configKeys.XP_DISPLAY_MODE) then
        ...
        if configManager.getBoolean(configKeys.VIP_SYSTEM_ENABLED) then
            ...
            if vipBonusExp > 0 and player:isVip() then
                ...
                player:sendTextMessage(MESSAGE_BOOSTED_CREATURE, "Normal base xp is: " .. baseRate ..
                    "%, because you are VIP, bonus of " .. vipBonusExp .. "%")
            end
        end
