...
function playerDeath.onDeath(player, corpse, killer, mostDamageKiller, unjustified, mostDamageUnjustified)
    ...
    if mostDamageKiller ~= nil then
        ...
        mostDamageName = mostDamageKiller:isMonster() and mostDamageKiller:getType():getNameDescription() or
    ...
    if player:getLevel() > protectionLevel then
        if player:hasBlessing(9) then
            hasBlessOfLife = 1
        end
    end
    ...
    db.query(
        "INSERT INTO `player_deaths` (`player_id`, `time`, `level`, `killed_by`, `is_player`, `mostdamage_by`, `mostdamage_is_player`, `unjustified`, `mostdamage_unjustified`, `playerWasRevived`) VALUES (" ..
            ...
            (unjustified and 1 or 0) .. ", " .. (mostDamageUnjustified and 1 or 0) .. "," .. hasBlessOfLife .. ")")
    ...
    if player:getLevel() > protectionLevel then
        if hasBlessOfLife == 0 then
            local home = getHouseByPlayerGUID(playerGuid)
            if home then
            ...
            if killerGuildId ~= 0 and targetGuildId ~= killerGuildId and isInWar(player:getId(), killer:getId()) then
                ...
                if warId then
                    ...
                    resultId = db.storeQuery(
                        "SELECT `guild_wars`.`id`, `guild_wars`.`frags_limit`, (SELECT COUNT(1) FROM `guildwar_kills` \z
						...
						FROM `guild_wars` WHERE (`guild1` = " .. killerGuildId .. " OR `guild2` = " .. killerGuildId ..
                            ") AND `status` = 1 AND `id` = " .. warId)
                    ...
                    if resultId then
                        ...
                        for i = 1, #members do
                            members[i]:sendChannelMessage(members[i],
                                string.format("%s was killed by %s. The new score is: %s %d:%d %s (frags limit: %d)",
                                    playerName, killerName, targetGuild:getName(), guild1_kills, guild2_kills,
                                    killerGuild:getName(), frags_limit), TALKTYPE_CHANNEL_R1, CHANNEL_GUILD)
                        end

                        local enemyMembers = targetGuild:getMembersOnline()
                        for i = 1, #enemyMembers do
                            enemyMembers[i]:sendChannelMessage(enemyMembers[i],
                                string.format("%s was killed by %s. The new score is: %s %d:%d %s (frags limit: %d)",
                                    playerName, killerName, targetGuild:getName(), guild1_kills, guild2_kills,
                                    killerGuild:getName(), frags_limit), TALKTYPE_CHANNEL_R1, CHANNEL_GUILD)
                        end
                        ...
                        if guild1_kills >= frags_limit or guild2_kills >= frags_limit then
                            db.query("UPDATE `guild_wars` SET `status` = 4, `ended` = " .. os.time() ..
                                         " WHERE `status` = 1 AND `id` = " .. warId)
                            Game.broadcastMessage(string.format("%s has just won the war against %s.",
                                killerGuild:getName(), targetGuild:getName()))
                        end
                    end
                end
            end
