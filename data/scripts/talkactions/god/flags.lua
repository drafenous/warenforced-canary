local PlayerFlags_t = {
    ...
    ["CannotChangeOutfit"] = CannotChangeOutfit,
...
function Player.talkactionHasFlag(self, param, flagType)
    ...
    if not targetPlayer:hasGroupFlag(flagValue) then
        self:sendTextMessage(MESSAGE_EVENT_ADVANCE,
            "Player " .. playerName .. " not have flag type: " .. getFlagNameByType(flagValue) .. ".")
    else
        self:sendTextMessage(MESSAGE_EVENT_ADVANCE,
            "Player " .. playerName .. " have flag type: " .. getFlagNameByType(flagValue) .. ".")
    end
...
function Player.talkactionSetFlag(self, param, flagType)
    ...
    if targetPlayer:hasFlag(flagValue) then
        self:sendTextMessage(MESSAGE_EVENT_ADVANCE,
            "Player " .. playerName .. " already has flag " .. getFlagNameByType(flagValue) .. ".")
        return true
    end

    targetPlayer:setGroupFlag(flagValue)
    self:sendTextMessage(MESSAGE_EVENT_ADVANCE,
        "Flag " .. getFlagNameByType(flagValue) .. " set for player " .. playerName .. ".")
    logger.info("[Player.talkactionSetFlag] Added flag {} to {} character by {}.", getFlagNameByType(flagValue),
        targetPlayer:getName(), self:getName())
    return true
end
...
function Player.talkactionRemoveFlag(self, param, flagType)
    ...
    if not targetPlayer:hasFlag(flagValue) then
        self:sendTextMessage(MESSAGE_EVENT_ADVANCE,
            "Player " .. playerName .. " not have flag " .. getFlagNameByType(flagValue) .. ".")
        return true
    end

    targetPlayer:removeGroupFlag(flagValue)
    self:sendTextMessage(MESSAGE_EVENT_ADVANCE,
        "Flag " .. getFlagNameByType(flagValue) .. " removed from player " .. playerName .. ".")
    logger.info("[Player.talkactionRemoveFlag] Removed flag {} to {} character by {}.", getFlagNameByType(flagValue),
        targetPlayer:getName(), self:getName())
    return true
end
