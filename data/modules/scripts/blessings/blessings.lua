...
Blessings.Credits = {
    ...
    todo = {"Insert & Select query in blessings_history",
            "Add unfair fight reductio (convert the get killer is pvp fight with getDamageMap of dead player)",
            "Gamestore buy blessing", "Test ank print text", "Test all functions", "Test henricus prices/blessings",
            ...
            "add blessings module support npc\\lib\\npcsystem\\modules.lua", "Fix store buying bless",
...
Blessings.sendBlessDialog = function(player)
    ...
    msg:addByte(Blessings.Config.HasToF and #Blessings.All - 1 or (#Blessings.All - 2)) -- total blessings
    for k = 1, #Blessings.All - 1 do
        v = Blessings.All[k]
        if v.type ~= Blessings.Types.PvP and v.type ~= Blessings.Types.CUSTOM or Blessings.Config.HasToF then
...
Blessings.doAdventurerBlessing = function(player)
    ...
    player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have adventurer's blessings for being level lower than " ..
...
Blessings.getInquisitionPrice = function(player)
    ...
    local totalBlessPrice = Blessings.getBlessingsCost(player:getLevel(), false) * missing *
...
Blessings.DropLoot = function(player, corpse, chance, skulled)
    ...
    for i = CONST_SLOT_HEAD, CONST_SLOT_AMMO do
        ...
        if item then
            ...
            Blessings.DebugPrint(thisChance / multiplier .. "%" .. " | thisRandom " .. thisRandom / multiplier .. "%",
                "DropLoot item " .. item:getName() .. " |")
...
Blessings.BuyAllBlesses = function(player)
    if not Tile(player:getPosition()):hasFlag(TILESTATE_PROTECTIONZONE) and
        (player:isPzLocked() or player:getCondition(CONDITION_INFIGHT, CONDITIONID_DEFAULT)) then
    ...
    if player:removeMoneyBank(totalCost) then
        ...
        player:sendCancelMessage("You received the remaining " .. missingBlessAmt .. " blesses for a total of " ..
                                     totalCost .. " gold.")
...
Blessings.PlayerDeath = function(player, corpse, killer)
    local hasToF = Blessings.Config.HasToF and player:hasBlessing(1) or false
    local hasAol = (player:getSlotItem(CONST_SLOT_NECKLACE) and player:getSlotItem(CONST_SLOT_NECKLACE):getId() ==
                       ITEM_AMULETOFLOSS)
