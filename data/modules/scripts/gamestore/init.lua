...
function parseTransferableCoins(playerId, msg)
    ...
    db.query("UPDATE `accounts` SET `coins_transferable` = `coins_transferable` + " .. amount .. " WHERE `id` = " ..
                 accountId)
...
function parseRequestStoreOffers(playerId, msg)
    ...
    elseif actionType == GameStore.ActionType.OPEN_USEFUL_THINGS then
        ...
        local category = nil
        if subAction >= GameStore.SubActions.BLESSING_TWIST and subAction <= GameStore.SubActions.BLESSING_ALL_PVP or
            subAction == GameStore.SubActions.BLESSING_OF_LIFE then
...
function parseBuyStoreOffer(playerId, msg)
    ...
    or not player or (player:getVocation():getId() == 0) and (not GameStore.haveOfferRook(id)) -- we don't have such offer
    or not offer or (offer.type == GameStore.OfferTypes.OFFER_TYPE_NONE) -- offer is disabled
    or
        (offer.type ~= GameStore.OfferTypes.OFFER_TYPE_NAMECHANGE and offer.type ~=
            GameStore.OfferTypes.OFFER_TYPE_EXPBOOST and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_PREYBONUS and
            offer.type ~= GameStore.OfferTypes.OFFER_TYPE_PREYSLOT and offer.type ~=
            GameStore.OfferTypes.OFFER_TYPE_HUNTINGSLOT and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_TEMPLE and
            offer.type ~= GameStore.OfferTypes.OFFER_TYPE_SEXCHANGE and offer.type ~=
            GameStore.OfferTypes.OFFER_TYPE_INSTANT_REWARD_ACCESS and offer.type ~=
            GameStore.OfferTypes.OFFER_TYPE_HIRELING and offer.type ~=
            GameStore.OfferTypes.OFFER_TYPE_HIRELING_NAMECHANGE and offer.type ~=
            GameStore.OfferTypes.OFFER_TYPE_HIRELING_SEXCHANGE and offer.type ~=
            GameStore.OfferTypes.OFFER_TYPE_HIRELING_SKILL and offer.type ~=
            GameStore.OfferTypes.OFFER_TYPE_HIRELING_OUTFIT and not offer.id) then
...
function Player.canBuyOffer(self, offer)
    ...
    if offer.type ~= GameStore.OfferTypes.OFFER_TYPE_NAMECHANGE and offer.type ~=
        ...
        offer.type ~= GameStore.OfferTypes.OFFER_TYPE_HUNTINGSLOT and offer.type ~=
        GameStore.OfferTypes.OFFER_TYPE_PREYBONUS and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_TEMPLE and
        offer.type ~= GameStore.OfferTypes.OFFER_TYPE_SEXCHANGE and offer.type ~=
        GameStore.OfferTypes.OFFER_TYPE_HIRELING_SKILL and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_HIRELING_OUTFIT and
        not offer.id then
        disabled = 1
    end
...
function sendShowStoreOffersOnOldProtocol(playerId, category)
    ...
    for _, offer in ipairs(category.offers) do
        if count > 0 and offer.coinType == GameStore.CoinType.Coin then
            ...
            local disabled, disabledReason = player:canBuyOffer(offer).disabled,
                player:canBuyOffer(offer).disabledReason
            local offerPrice = offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST and
                                   GameStore.ExpBoostValues[player:getStorageValue(GameStore.Storages.expBoostCount)] or
        ...
        if category.offers then
            for Off_k, offer in ipairs(category.offers) do
                ...
                elseif type(offer.id) == "table" and
                    (offer.type == GameStore.OfferTypes.OFFER_TYPE_OUTFIT or offer.type ==
                        GameStore.OfferTypes.OFFER_TYPE_OUTFIT_ADDON) then
                    for m, offerId in pairs(offer.id) do
                        -- in case of outfits we have offer.id = {male = ..., female = ...}
                        if offerId == id then
...
GameStore.retrieveHistoryEntries = function(accountId, currentPage, entriesPerPage)
    ...
    local resultId = db.storeQuery("SELECT * FROM `store_history` WHERE `account_id` = " .. accountId ..
...
function GameStore.processInstantRewardAccess(player, offerCount)
    ...
    if player:getCollectionTokens() + offerCount >= limit + 1 then
        return error({
            code = 1,
            message = "You cannot own more than " .. limit .. " reward tokens."
        })
    end
    ...
    if inbox then
        ...
        while remainingCount > 0 do
            ...
            if inboxItem then
                ...
                return error({
                    code = 0,
                    message = "Error adding item to store inbox."
                })
            end
...
function GameStore.processPreyBonusReroll(player, offerCount)
    ...
    if player:getPreyCards() + offerCount >= limit + 1 then
        return error({
            code = 1,
            message = "You cannot own more than " .. limit .. " prey wildcards."
        })
    end
